...
- New installations default to `danger-full-access` with approvals off. This allows access to files and network services available to the Linux user. Choose `workspace-write` or `read-only` during setup if you need a restricted sandbox. Upgrades preserve your existing selection.
